	...
	<div class="flex h-full flex-col overflow-y-auto">
		...
		<div class="border-b border-border px-3 py-2.5">
			<div class="flex items-center gap-2">
				<div class="min-w-0 flex-1 truncate text-xs font-semibold text-foreground">{clip.name}</div>
				<Button
					variant="ghost"
					size="icon-xs"
		...
		<div class="space-y-4 px-3 py-3">
			...
			{#if !isAudioClip}
				<GradingPanel
					...
					{onMatchColor}
					{matching}
					onAutoLevels={clip?.assetId ? onAutoLevels : undefined}
					{autoLeveling}
					onLutPreview={clip?.assetId ? onLutPreview : undefined}
				/>
			{/if}
